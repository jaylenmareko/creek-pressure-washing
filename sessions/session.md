...
## 2026-07-27 (cont'd) — De-duplicated location pages + fixed a live 404 bug

- **Did:** Jaylen asked whether the site is actually tailored to rank in Wichita search. Answered honestly: on-page tailoring helps but doesn't move local-pack rankings — that's dominated by Google Business Profile (not set up), reviews (none), citations, and proximity (legal address is 45 min away in Winfield). Flagged the 8 location pages as near-duplicate templates, a doorway-page risk.
- **Fixed the doorway-page risk:** Added a unique "Pressure Washing in [City], KS" section (real direction/county/zip relative to Wichita, housing character) + a 2-question FAQ to each of the 8 location pages. Facts checked via web search (zip codes, county, growth character) rather than invented.
- **Found and fixed a live bug while verifying:** curled the production site to confirm the redeploy — every location page's canonical URL, OG url, sitemap.xml entry, and footer link (`/andover-ks` etc.) was 404ing in production. Vercel was only serving the `.html` path. Added `"cleanUrls": true` to `vercel.json`. Confirmed all 8 clean URLs return 200 post-deploy. This was a pre-existing bug (not introduced this session) but was silently killing every canonical/sitemap URL until now.
- **Still not done (flagged to Jaylen, his call):** Google Business Profile setup — the single biggest lever for "popping up first" in local search, bigger than any site copy. Also: reviews, directory citations (Yelp/Angi/BBB/Nextdoor), and the Winfield-address proximity handicap for Wichita local-pack ranking.
